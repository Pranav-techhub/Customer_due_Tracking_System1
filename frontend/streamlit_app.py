...
# ---------------- ---------- LOGGED IN ----------------
    ...
    if role == "admin":
        tabs = {
            "➕ Add Customer": "add",
            "🔑 Manage Credentials": "credentials",  # NEW TAB
        ...
        if choice == "➕ Add Customer":
            ...
            if st.button("Add Customer"):
                ...
                if validate_customer(name, phone, email, df):
                    ...
                    # In the "Add Customer" section, replace the credential generation with:
                    username = re.sub(r'[^a-zA-Z0-9]', '', name).lower()
                    random_digits = ''.join(secrets.choice(string.digits) for _ in range(4))
                    plain_password = f"{name.replace(' ', '')}{random_digits}"
        ...
        if choice == "💰 Pay Due":
            ...
            if 'customer_id' not in st.session_state:       
                ...
                **Account Summary:**
                ...
                # ---------- Option 2: Pay custom amount ----------
                st.subheader("Pay Now")
                amount = st.number_input(
                    "Enter Payment Amount",
                    min_value=0.0,
                    max_value=float(current_due),
                    value=min(1000.0, float(current_due)),
                    format="%.2f"
                )

                if st.button("Pay Due"):
                    if amount <= 0:
                        st.error("Amount must be greater than 0")
                    else:
                        updated = services.user_pay_due(
                            username=st.session_state.username,
                            customer_id=cust_id,
                            amount=amount
                        )
                        if updated:
                            st.session_state.customer_due = updated['due']
                            st.success(f"""
                            Payment of ₹{amount:.2f} successful!
                            Remaining due: ₹{updated['due']:.2f}
                            """)
                        else:
